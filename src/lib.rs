...
impl PCA9685 {
    ...
    pub async fn begin(&mut self, prescale: u8 ) -> Result<(), i2c::Error> {
        ...
        match prescale {
            p if p == 0 => {
                println!("Setting Freq to device");
                if let Err(e) = self.set_pwm_freq(1000.0).await {
                    return Err(e);
                }
            }
    ...
    pub async fn reset(&mut self) -> Result<(), rppal::i2c::Error> {
        if let Err(e) = self.bus.write(&vec![PCA9685_MODE1, MODE1_RESTART]) {
            return Err(e);
        }
    ...
    pub async fn set_pwm_freq(&mut self, freq: f32) -> Result<(), rppal::i2c::Error> {
        ...
        println!("Write Read_set_pwm_freq: {:?}", buf);
    ...
    pub fn get_prescale(&mut self) -> (usize, i2c::Result<()>) {
        ...
        println!("Write Read_getprescale: {:?}", buf);
    ...
    pub async fn set_ext_clock(&mut self, prescale: u8) -> Result<(), rppal::i2c::Error> {
        ...
        println!("Write Read_setextclock: {:?}", buf);
        if let Err(e) = result {
            return Err(e);
        }
    ...
    pub fn set_output_mode(&mut self, totempole: bool) -> Result<(), rppal::i2c::Error> {
        ...
        println!("Write Read_setoutputmode: {:?}", buf);
